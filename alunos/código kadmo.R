...
# Paths
root     <- "C:/Users/Kadmo Managem/Desktop/BE_STP/"
...
base_alunos<- readRDS(paste0(tmp,"alunos_todos_v2.RData")) %>% 
  ...
  mutate(nome_da_escola10=gsub("º","", nome_da_escola9)) %>% 
  mutate(nome_da_escola11=gsub("Almas 1 ciclo","", nome_da_escola10)) %>%
  mutate(nome_da_escola100=gsub("()","", nome_da_escola11))

#%>% 
#  filter(distrito=="Mé-Zochi") %>% 
#  dplyr::select(nome_da_escola100) %>% 
#  distinct(nome_da_escola100, .keep_all = TRUE)
...
base_codigos <- clean_names(read_xlsx(paste0(input,
  ...
  mutate(nome_da_escola7=gsub("Básica","", nome_da_escola6)) %>% 
  mutate(nome_da_escola8=gsub("Escola","", nome_da_escola7)) %>% 
  mutate(nome_da_escola9=gsub("Manuel do Rosário","Manuel do Rosário ()", nome_da_escola8)) %>% 
  mutate(nome_da_escola10=gsub("M.Q.B.(BATEPÀ)","Manuel Quaresma Bragança (BATEPÀ)S", nome_da_escola9)) %>%
  mutate(nome_da_escola100=stringr::str_trim(nome_da_escola10)) %>% 
...
base_alunos_cod <- base_alunos %>% 
  filter(distrito=="Mé-Zochi") %>% 
  dplyr::select(nome_da_escola100, distrito, nome_do_aluno, classe) %>% 
  distinct(nome_da_escola100, .keep_all = TRUE) %>% 
  anti_join(base_codigos,
            by="nome_da_escola100")
